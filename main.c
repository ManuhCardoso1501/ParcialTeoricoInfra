...
void aplicarFiltro(int *imagen, int *imagenProcesada, int width, int height, int numHilos);

// Función para calcular la suma de los píxeles (como una estadística)
int calcularSumaPixeles(int *imagen, int width, int height, int numHilos);
...
int main(int argc, char* argv[]) {
    ...
    // Obtener el número de núcleos de la máquina
    int numCores = omp_get_num_procs();
    int numHilosDobles = numCores*2;
    //printf("Número de núcleos: %d\n", numCores); Para conocer el número de hilos
    //printf("Número de núcleos para segundo caso: %d\n", numHilosDobles); Para conocer el número de hilos * 2

    // Aplicar filtro (paralelizable)
    //aplicarFiltro(imagen, imagenProcesada, width, height, numCores); Para el caso de secuencial
    aplicarFiltro(imagen, imagenProcesada, width, height, numHilosDobles);

    // Calcular suma de píxeles (parte paralelizable)
    //int sumaPixeles = calcularSumaPixeles(imagenProcesada, width, height, numCores); Para el caso de secuencial
    int sumaPixeles2 = calcularSumaPixeles(imagenProcesada, width, height, numHilosDobles);
    //printf("Suma de píxeles: %d\n", sumaPixeles);
    printf("Suma de píxeles Cores*2: %d\n", sumaPixeles2);
...
void aplicarFiltro(int *imagen, int *imagenProcesada, int width, int height, int numHilos) {
    ...
    #pragma omp parallel for num_threads(numHilos) collapse(2) schedule(dynamic)
...
int calcularSumaPixeles(int *imagen, int width, int height, int numHilos) {
    int suma = 0;
    #pragma omp parallel for num_threads(numHilos) reduction(+:suma) schedule(dynamic)
